...
QString compactScoreDetails(const QJsonObject &scoreDetails)
{
    int scores[10] = {};
...
int determineSubjectType(const int originalType, const QJsonArray &tags)
{   // 类型处理
...
{   // episode公共数据插入
    ...
    QSqlQuery query(db);
    query.prepare("INSERT OR REPLACE INTO episode_public_date VALUES (?,?,?,?,?)");
    db.transaction();
    int count = 0;
    ...
    while (!stream.atEnd()) {
        ...
        query.addBindValue(obj["subject_id"].toInt());
        query.addBindValue(obj["id"].toInt());
        query.addBindValue(dateStringToTimestamp(airdateStr));
        query.addBindValue(static_cast<int>(obj["sort"].toDouble() * 10.0));
        query.addBindValue(obj["type"].toInt());
        query.exec();
        ++count;
...
{   // subject公共数据插入
    ...
    while (!stream.atEnd()) {
        ...
        query.addBindValue(determineSubjectType(type, tags));
        ...
        query.addBindValue(compactScoreDetails(obj["score_details"].toObject()));
...
bool insertSubjectCharacter(const QString &filePath, QSqlDatabase db)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) return false;
    QSqlQuery query(db);
    query.prepare("INSERT OR REPLACE INTO subject_character VALUES (?,?,?)");
    db.transaction();
    int count = 0;
    constexpr int batchSize = 10000;
    QTextStream stream(&file);
...
bool insertPerson(const QString &filePath, QSqlDatabase db)
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) return false;
    QSqlQuery query(db);
    query.prepare("INSERT OR REPLACE INTO person_public_date VALUES (?,?,?)");
    db.transaction();
    int count = 0;
    constexpr int batchSize = 10000;
    QTextStream stream(&file);
...
bool insertPersonCharacter(const QString &filePath, QSqlDatabase db)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) return false;
    QSqlQuery query(db);
    query.prepare("INSERT OR REPLACE INTO person_character VALUES (?,?,?)");
    db.transaction();
    int count = 0;
    constexpr int batchSize = 10000;
    QTextStream stream(&file);
...
bool insertSubjectPersons(const QString &filePath, QSqlDatabase db)
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) return false;
    QSqlQuery query(db);
    query.prepare("INSERT OR REPLACE INTO subject_persons VALUES (?,?,?)");
    db.transaction();
    int count = 0;
    constexpr int batchSize = 10000;
    QTextStream stream(&file);
    while (!stream.atEnd()) {
        ...
        if (parseError.error != QJsonParseError::NoError || !doc.isObject()) continue;
        QJsonObject obj = doc.object();
        const int position = obj["position"].toInt();
        if (position >= 3000) continue;
        query.addBindValue(obj["subject_id"].toInt());
        query.addBindValue(obj["person_id"].toInt());
...
bool insertSubjectRelations(const QString &filePath, QSqlDatabase db)
{
    static const QSet allowedTypes = {2, 3, 4, 5, 6, 11, 1003};
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) return false;
    QSqlQuery query(db);
    query.prepare("INSERT OR REPLACE INTO subject_relations VALUES (?,?,?)");
    db.transaction();
    int count = 0;
    constexpr int batchSize = 10000;
    QTextStream stream(&file);
    while (!stream.atEnd()) {
        ...
        QJsonObject obj = doc.object();
        int subject_id = obj["subject_id"].toInt();
        int related_subject_id = obj["related_subject_id"].toInt();
        int relation_type = obj["relation_type"].toInt();
        if (!allowedTypes.contains(relation_type)) continue;
        query.addBindValue(subject_id);
        query.addBindValue(related_subject_id);
        query.addBindValue(relation_type);
        query.exec();
        ++count;
        if (count % batchSize == 0) {
            db.commit();
...
bool extractZip(const QString &zipPath, const QString &destDir, QString &episodePath, QString &subjectPath, QString &characterPath, QString &subjectCharacterPath, QString &personPath, QString &personCharacterPath, QString &subjectPersonsPath, QString &subjectRelationsPath)
{
    const QDir dir;
    ...
    subjectPersonsPath = destDir + "/subject-persons.jsonlines";
...
int main(int argc, char *argv[])
{
    ...
    QString episodeFile, subjectFile, characterFile, subjectCharacterFile, personFile, personCharacterFile, subjectPersonsFile, subjectRelationsFile;
    if (!extractZip(zipPath, extractDir, episodeFile, subjectFile, characterFile, subjectCharacterFile, personFile, personCharacterFile, subjectPersonsFile, subjectRelationsFile)) return 1;
    ...
    for (int i = 0; i < typeCombinations.size(); ++i) {
        ...
        tasks.append([=] {
            const QString connName = QString("public_date_connection_%1").arg(reinterpret_cast<quintptr>(QThread::currentThreadId()));
            {
                ...
                if (!insertSubjectPersons(subjectPersonsFile, db)) {
                    qDebug() << QThread::currentThreadId() << "插入 Subject_Persons 失败";
                    db.close();
                    QSqlDatabase::removeDatabase(connName);
                    return;
                }
                if (!insertSubjectRelations(subjectRelationsFile, db)) {
                    qDebug() << QThread::currentThreadId() << "插入 Subject_Relations 失败";
                    db.close();
                    QSqlDatabase::removeDatabase(connName);
                    return;
                }
